...
@Service
public class IOrderServiceIpml implements IOrderService{
    ...
    * */
    ...
    public ServerResponse create(Integer userId, Integer shippingId) {
       //参数非空校验
        if(shippingId==null)
        {
            return ServerResponse.createServerRespnseByError("地址参数不能为空");
        }
        //根据用户ID查询购物车已选中的商品
        List<Cart> cartList = cartMapper.findCarListByUserIdAndChecked(userId);
        //将cart转成CARTVO对象
        ServerResponse serverResponse = getCartPrderItem(userId,cartList);
        ...
        {
            return ServerResponse.createServerRespnseByError("购物车为空");
        }
       ...
       if(order==null)
       {
           return ServerResponse.createServerRespnseByError("订单创建失败");
       }
       //将OrderItem保存到数据库
       ...
       //清空购物车已经选中的商品
        ...
        return ServerResponse.createServerRespnseBySucces(orderVO);
    }
    ...
    * */
    private OrderVO assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId)
    {
      ...
      orderVO.setPostage(0);
        ...
        Const.PaymetEnum paymetEnum = Const.PaymetEnum.codeOf(order.getPaymentType());
        if (paymetEnum!=null)
        {
  ...
  private OrderItemVO assembleOrderItemVO(OrderItem orderItem)
  {
    ...
    * */
    ...
    private void cheanCart(List<Cart> cartList)
    {
    ...
    * */
    private BigDecimal getOrderPrice(List<OrderItem> orderItemList){
        ...
        for (OrderItem orderItem:orderItemList) {
            bigDecimal  = BigDecimaUtils.add(bigDecimal.doubleValue(), orderItem.getTotalPrice().doubleValue());
        }
...
private Order createOrder(Integer userId,Integer shippingId,BigDecimal orderTotalPrice){
    ...
    //未付款
    order.setStatus(Const.OrderStatusEnum.ORDER_UP_PAY.getCode());
    //订单金额
    order.setPayment(orderTotalPrice);
    //邮费
    order.setPostage(0);
    //支付类型
    order.setPaymentType(Const.PaymetEnum.ONLINE.getCode());
    ...
    private ServerResponse getCartPrderItem(Integer userId,List<Cart> cartList){
        //判断购物车是否为空
            if(cartList==null||cartList.size()==0)
            {
                return ServerResponse.createServerRespnseByError("购物车为空");
           ...
           for (Cart cart:cartList) {
               ...
               Product product = productMapper.selectByPrimaryKey(cart.getProductId());
                 if(product==null)
                 {
                     return ServerResponse.createServerRespnseByError("id为"+cart.getProductId()+"的商品不存在");
                 }
                 //判断商品是否下架
                 if(product.getStatus()!= Const.ProductStatusEnum.PRODUCT_ONLINE.getCode())
                 {
                     return ServerResponse.createServerRespnseByError("id为"+product.getId()+"的商品已下架");
                 }
                 //库存不足
                 if(product.getStock()<cart.getQuantity())
                 {
                     return  ServerResponse.createServerRespnseByError("id为"+product.getId()+"的商品库存不足");
                 }
                 ...
                 orderItem.setTotalPrice(BigDecimaUtils.mul(product.getPrice().doubleValue(),cart.getQuantity().doubleValue()));

               orderItemList.add(orderItem);
           }
            return ServerResponse.createServerRespnseBySucces(orderItemList);
       }
    ...
    //根据用户ID和orderno查询订单
        Order order = orderMapper.findOrderByUseridAndrderNo(userId, orderNo);
    if(order==null)
    {
        return ServerResponse.createServerRespnseByError("订单不存在");
    }
    //判断订单状态并取消
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_UP_PAY.getCode())
        {
            return ServerResponse.createServerRespnseByError("订单不可取消");
        }
        ...
        return ServerResponse.createServerRespnseByError("订单取消失败");
    }
/*
* 获取购物车中订单明细
* */
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        //查询购物车
        List<Cart> carList = cartMapper.findCarListByUserIdAndChecked(userId);
        ServerResponse serverResponse = getCartPrderItem(userId, carList);
        if(!serverResponse.isSuccess())
        {
            return serverResponse;
        }
        ...
        return ServerResponse.createServerRespnseBySucces(cartOrderItemVO);
    }
/*
* 订单列表
* */
    ...
    public ServerResponse list(Integer userId,Integer pageSize,Integer pageNum) {
        ...
        {
            return ServerResponse.createServerRespnseByError("未查询到订单信息");
        }
...
* */
    ...
    public ServerResponse detail(Long orderNo) {
        ...
        //查询订单
        Order order = orderMapper.findOrderAndrderNo(orderNo);
        if(order==null)
        {
            return ServerResponse.createServerRespnseByError("订单不存在");
        }
...
///////////////////////////支付////////////////
    ...
    * */
    ...
    public ServerResponse pay(Integer userId, Long orderNo,String path) {
        if (orderNo==null)
        {
            return ServerResponse.createServerRespnseByError("订单号不能为空");
        }
        //测试当面生成二维码
        Order order = orderMapper.findOrderAndrderNo(orderNo);
        ...
        try {
            ServerResponse pay = pay(order,path);
            return pay;
        } catch (IOException e) {
            e.printStackTrace();
        }
        return ServerResponse.createServerRespnseByError("下单失败");
    }
    ...
    * */
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
      ...
      //获取流水号
        ...
        if (order==null)
        {
            return ServerResponse.createServerRespnseByError("订单"+orderNo+"不是本商品的订单");
        }
        if(order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode())
        {
            //防止支付宝重复回调
            return ServerResponse.createServerRespnseByError("支付宝重复调用");
        }
        ...
        payInfo.setPayPlatform(Const.PaymetPlatformEnum.ALIPAY.getCode());
        ...
        return ServerResponse.createServerRespnseByError("支付信息保存失败");
    }
/*
* 查询订单支付状态
* */
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        ...
        if(order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode())
        {
            return ServerResponse.createServerRespnseBySucces(true);
        }
...
private static Log log = LogFactory.getLog(Main.class);
    ...
    // 测试当面付2.0生成支付二维码
    public ServerResponse pay(Order order,String path) throws IOException {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://59.110.125.78:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
